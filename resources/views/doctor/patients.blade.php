        ...
        @if ($patients->count() > 0)
            <div class="overflow-x-auto">
                <table class="min-w-full bg-white rounded-lg">
                    <thead>
                        <tr
                            class="bg-gray-100 border-b border-gray-200 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">
                            ...
                            <tr class="hover:bg-gray-50 border-b border-gray-200">
                                <td class="px-6 py-4">
                                    <div class="flex items-center space-x-3">
                                        <div
                                            class="h-10 w-10 rounded-full bg-blue-100 flex items-center justify-center overflow-hidden">
                                            @if ($patient->image)
                                                <img src="{{ $patient->image }}" alt="Patient photo"
                                                    class="h-full w-full object-cover">
                                            @else
                                                <span
                                                    class="text-blue-500 font-semibold">{{ substr($patient->first_name, 0, 1) }}</span>
                                            @endif
                                        </div>
                                        <div>
                                            <p class="font-medium text-gray-800">{{ $patient->first_name }}
                                                {{ $patient->last_name }}</p>
        ...
        @else
            <div class="text-center py-16">
                ...
                <a href="{{ route('doctor.appointments') }}"
                    class="px-4 py-2 bg-accent hover:bg-accentHover text-white rounded-lg transition-colors duration-200">
                    Manage Appointments
                </a>
            </div>
        @endif
