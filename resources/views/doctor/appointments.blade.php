                ...
                <div class="flex flex-col sm:flex-row gap-4">
                    ...
                    <div class="relative">
                        <select id="status-filter"
                            class="pl-4 pr-10 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-accent appearance-none bg-white">
                    ...
                    <!-- Date filter -->
                    <input type="date"
                        class="pl-4 pr-4 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-accent">
                </div>
            </div>
                                        ...
                                        <div class="h-10 w-10 flex-shrink-0">
                                            @if ($appointment->patient->user->image)
                                                <img class="h-10 w-10 rounded-full object-cover"
                                                    src="{{ $appointment->patient->user->image }}" alt="">
                                            @else
                                                <div
                                    ...
                                    <div class="flex space-x-2 justify-end">
                                        ...
                                        @if ($appointment->status == 'pending')
                                            <form action="{{ route('doctor.appointments.confirm', $appointment) }}"
                                                ...
                                                @csrf
                                                <button type="submit"
                                                    class="bg-green-100 hover:bg-green-200 text-green-700 rounded-md px-2 py-1 transition-colors"
                                                    title="Confirm Appointment">
                                                    <i class="fas fa-calendar-check mr-1"></i> confirm
                                                </button>
                                            </form>

                                            <form action="{{ route('doctor.appointments.reject', $appointment) }}"
                                                method="POST" class="inline-block">
                                                @csrf
                                                <button type="submit"
                                                    class="bg-red-100 hover:bg-red-200 text-red-700 rounded-md px-2 py-1 transition-colors"
                                        ...
                                        @if ($appointment->status == 'upcoming')
                                            <form action="{{ route('doctor.appointments.complete', $appointment) }}"
                                                ...
                                                @csrf
                                                <button type="submit"
                                                    class="bg-blue-100 hover:bg-blue-200 text-blue-700 rounded-md px-2 py-1 transition-colors"
